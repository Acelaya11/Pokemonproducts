...
export const updateCategories = (currentCategories: Category[], categoryId: string): Category[] => {
  ...
  if (categoryId === 'AllExtra') {
    const allExtraCategories = currentCategories.filter(cat => cat.type === 'extra' && cat.id !== 'AllExtra');
    const isAllExtraChecked = currentCategories.find(cat => cat.id === 'AllExtra')?.checked;
    
    console.log('🔍 DEBUG: AllExtra handler:', { isAllExtraChecked, allExtraCategories: allExtraCategories.map(cat => ({ id: cat.id, checked: cat.checked })) });
  ...
  if (clickedCategory) {
    ...
    console.log('🔍 DEBUG: Processing category:', { categoryId, clickedCategoryType, isAllOption: categoryId === getAllOptionId(clickedCategoryType) });
    ...
    if (isCheckingSpecificOption) {
      ...
      console.log('🔍 DEBUG: Auto-reset check for', clickedCategoryType, ':', {
        allSpecificOptions: allSpecificOptions.map(cat => ({ id: cat.id, checked: cat.checked })),
        selectedSpecificOptions: selectedSpecificOptions.map(cat => ({ id: cat.id, checked: cat.checked })),
        shouldAutoReset: allSpecificOptions.length > 0 && selectedSpecificOptions.length === allSpecificOptions.length
      });
      ...
      if (allSpecificOptions.length > 0 && selectedSpecificOptions.length === allSpecificOptions.length) {
        const allOptionId = getAllOptionId(clickedCategoryType);
        if (allOptionId) {
          console.log('🔍 DEBUG: Auto-resetting to "All" for', clickedCategoryType, ':', allOptionId);
          updatedCategories = updatedCategories.map(cat => {
